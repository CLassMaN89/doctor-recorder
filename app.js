...
function renderDashboard(d){
 ...
 row.innerHTML=`<div class="rec-id"><span class="rec-dot"></span><b class="rec-txt">REC</b><img class="rec-anim" src="live-recording.svg" alt="" draggable="false"><span>${numOf.get(r.id)}</span></div><div class="rec-doctor"><b>${esc(x.doctor_first_name||'Eski kayıt')} ${esc(x.doctor_last_name||'')}</b><button class="pin-rec" type="button" aria-pressed="${pins.includes(r.id)}" title="${pins.includes(r.id)?'Sabitlemeyi kaldır':'Üste sabitle'}" aria-label="${pins.includes(r.id)?'Sabitlemeyi kaldır':'Üste sabitle'}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 17v5M9 3h6l-1 6 3.2 3.4V14H6.8v-1.6L10 9z"/></svg></button></div><div class="rec-device">${osIcon(x.device_model)}<span class="rd-model" title="${esc(x.device_model||'Telefon')}">${esc(x.device_model||'Telefon')}</span>${x.ip_address?`<span class="ip-badge">${esc(x.ip_address)}</span>`:''}</div><div class="rec-date">${stamp}</div><div class="rec-duration">${fmt(r.duration_seconds)}</div><div class="wave-player">${waveMarkup(r.id||r.file_path||stamp)}<button class="play" aria-label="Oynat"></button><button class="delete-rec" title="Kaydı sil" aria-label="Kaydı sil"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"/></svg></button><audio preload="metadata" src="${r.signed_url||''}"></audio></div>`;
